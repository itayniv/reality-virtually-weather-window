...
namespace DigitalRuby.RainMaker
{
    public class RainCollision : MonoBehaviour
    {
        ...
        private void Emit(ParticleSystem p, ref Vector3 pos)
        {
            ...
            while (count != 0)
            {
                float yVelocity = UnityEngine.Random.Range(0.2f, 1.0f);
                float zVelocity = UnityEngine.Random.Range(-1.0f, 1.0f);
                float xVelocity = UnityEngine.Random.Range(-1.0f, 1.0f);
                const float lifetime = 0.75f;// UnityEngine.Random.Range(0.25f, 0.75f);
                float size = UnityEngine.Random.Range(0.005f, 0.01f);
